...
class WidgetTester {
  ...
  async getTestData() {
    try {
      const users = await User.find({ email: { $regex: /@testwidget\.com$/i } }).limit(6);
      const projects = await Project.find({ title: /Widget Test.*Enhanced/i }).populate('members.user').limit(1);
      const tasks = await Task.find({ title: { $regex: /Critical|Security|Performance|Dashboard|Mobile|Integration|Documentation|Polish|Legacy/i } }).limit(15);
  ...
  testMilestonesWidget(testData) {
    ...
    try {
      ...
      widget.tests.push({
        ...
        status: testData.project.startDate && testData.project.deadline ? 'pass' : 'fail',
      ...
      const hasManagementRoles = testData.project.members.some(m => 
        ['supervisor', 'team-lead'].includes(m.role)
      );
      widget.tests.push({

...
const FilesWidget = ({ 
  ...
  const handleFileUpload = async (files) => {
    if (!isProjectCreator) {
      toast.error('Only project creators can upload files');
      return;
    }
  ...
  return (
    <div className={`widget-card ${className}`}>
      ...
      {/* Unlink Google Drive Button (Visible only to project creators) */}
      {isProjectCreator && googleDriveConnected && (
        <div className="mt-6">
          <button
            onClick={handleUnlinkGoogleDrive}
            className="w-full inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-red-700 bg-red-100 hover:bg-red-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-colors"
          >
            {googleDriveLoading ? (
              <>
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-all duration-200 group">
      <div className="p-4">
        ...
        <div>
          <h4 className="text-sm font-medium text-gray-900 dark:text-white truncate mb-1" title={file.name}>
            {file.name}
          </h4>
